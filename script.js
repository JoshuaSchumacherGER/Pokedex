...
async function fetchTranslatedData(url, language, fallbackLanguage = 'en') {
    try {
        ...
        const translatedData = data.names.find(name => name.language.name === language);
        if (translatedData) {
            return translatedData.name;
        } else {
            const fallbackData = data.names.find(name => name.language.name === fallbackLanguage);
...
async function fetchTranslatedAbility(url, language, fallbackLanguage = 'en') {
    try {
        const response = await fetch(url);
        const data = await response.json();
        const translatedAbility = data.names.find(name => name.language.name === language);
        if (translatedAbility) {
            return translatedAbility.name;
        } else {
            const fallbackAbility = data.names.find(name => name.language.name === fallbackLanguage);
...
async function fetchTranslatedType(url, language, fallbackLanguage = 'en') {
    try {
        const response = await fetch(url);
        const data = await response.json();
        const translatedType = data.names.find(name => name.language.name === language);
        if (translatedType) {
            return translatedType.name;
        } else {
            const fallbackType = data.names.find(name => name.language.name === fallbackLanguage);
...
async function fetchTranslatedMove(url, language, fallbackLanguage = 'en') {
    try {
        const response = await fetch(url);
        const data = await response.json();
        const translatedMove = data.names.find(name => name.language.name === language);
        if (translatedMove) {
            return translatedMove.name;
        } else {
            const fallbackMove = data.names.find(name => name.language.name === fallbackLanguage);
...
async function createPokemonCard(id, language, controller) {
    try {
        ...
        const translatedName = await fetchTranslatedData(data.species.url, language);

        // Skip if translation for name is not found
        if (!translatedName) {
            console.warn(`Translation not found for Pokemon ID: ${id} in language: ${language}`);
            return;
        }
        ...
        const abilities = await Promise.all(
            data.abilities.map(async (ability) => {
                const translatedAbility = await fetchTranslatedAbility(ability.ability.url, language);
        ...
        const types = await Promise.all(
            data.types.map(async (type) => {
                const translatedType = await fetchTranslatedType(type.type.url, language);
        ...
        const moves = await Promise.all(
            data.moves.slice(0, 3).map(async (move) => {
                const translatedMove = await fetchTranslatedMove(move.move.url, language);
        ...
        card.addEventListener('click', () => {
            ...
            const heldItems = data.held_items.length === 0 ? "/" : data.held_items.map(item => item.item.name).slice(0, 3).join(', ');
            ...
            document.getElementById("cries").src = data.cries ? data.cries.latest : ''; // Assuming cries URL exists in the data
